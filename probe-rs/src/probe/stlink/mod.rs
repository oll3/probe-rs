...
use super::{DebugProbe, DebugProbeError, WireProtocol};
use crate::architecture::arm::{ArmError, valid_32bit_arm_address};
use crate::{
    MemoryInterface,
    architecture::arm::{
        DapAccess, FullyQualifiedApAddress, Pins, SwoAccess, SwoConfig, SwoMode,
        ap::{
            AccessPortType,
    ...
    probe::{
        DebugProbeInfo, DebugProbeSelector, GpioConfig, GpioInterface, GpioMode, GpioNumber,
        GpioOutputType, GpioPull, GpioSpeed, Probe, ProbeError, ProbeFactory,
    },
};
...
impl GpioInterface for StLink<StLinkUsbDevice> {
    ...
    fn init_gpio(&mut self, configs: &[(GpioNumber, GpioConfig)]) -> Result<(), DebugProbeError> {
        ...
        for (gpio, config) in configs {
            if gpio.0 >= BRIDGE_NUM_GPIOS {
                Err(StlinkError::BridgeGpioOutOfRange)?;
            }
    ...
    fn write_gpio(&mut self, outputs: &[(GpioNumber, bool)]) -> Result<(), DebugProbeError> {
        ...
        if response[2] & mask != 0 {
            Err(StlinkError::BridgeGpioError)?;
        }
        Ok(())
    }
...
pub enum StlinkError {
    ...
    /// Bridge Command failed with status {0:?}.
    BridgeCommandFailed(BridgeStatus),

    /// No bridge support for this STLink.
    BridgeNotSupported,

    /// Bridge GPIO is outside valid range.
    BridgeGpioOutOfRange,
